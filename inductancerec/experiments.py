...
class ClassificationResults:
    ...
    def __init__(self, matching_alg_name, map_deformation_name_to_list_results, **kwargs):
        '''
        ...
        self.map_deformation_name_to_list_results = map_deformation_name_to_list_results
        ...
        for deformation_name, list_results in map_deformation_name_to_list_results.items():
    ...
    def get_deformation_names(self):
        '''Returns a sorted list of deformation names'''
        return sorted(self.map_deformation_name_to_list_results.keys())
    ...
    def get_num_correct_for_deformation(self, deformation_name):
        '''Returns the number correct for this deformation'''
        list_results_for_deformation = self.map_deformation_name_to_list_results[deformation_name]
        correctness_cnt = 0
        for result in list_results_for_deformation:
            if result.is_correct:
                correctness_cnt += 1
    ...
    def get_correct_match_scores_for_deformation(self, deformation_name):
        '''Returns a list of scores for the correct matches for this deformation'''
        results_for_deformation = self.map_deformation_name_to_list_results[deformation_name]
        correct_scores = [result.score for result in results_for_deformation if result.is_correct]
        return correct_scores
    
    def get_incorrect_match_scores_for_deformation(self, deformation_name):
        '''Returns a list of scores for the incorrect matches for this deformation'''
        results_for_deformation = self.map_deformation_name_to_list_results[deformation_name]
        incorrect_scores = [result.score for result in results_for_deformation if not result.is_correct]
        return incorrect_scores
    
    def get_correct_match_indices_in_nbestlist_for_deformation(self, deformation_name):
        '''Returns a list of correct match indices in the n-best list for the given deformation'''
        results = self.map_deformation_name_to_list_results[deformation_name]
    ...
    def get_confusion_matrix(self):
        ...
        y_true = list()
        y_pred = list()
        for deformation_name, list_results in self.map_deformation_name_to_list_results.items():
            for result in list_results:
                y_true.append(result.test_trial.deformation_name)
                y_pred.append(result.closest_trial.deformation_name)

        cm_classes = self.get_deformation_names()
    ...
    def get_nbestlist_performance(self, normalized=True):
        ...
        for deformation_name, results in self.map_deformation_name_to_list_results.items():
            for trial_classification_result in results:
                for i in range(0, len(trial_classification_result.n_best_list_sorted)):
                    ...
                    if trial_classification_result.test_trial.deformation_name == matched_trial.deformation_name:
    ...
    def print_result(self):
        ...
        for deformation_name in self.get_deformation_names():
            correctness_cnt_for_deformation = self.get_num_correct_for_deformation(deformation_name)
            num_comparisons_for_deformation = len(self.map_deformation_name_to_list_results[deformation_name])
            print("- {} {}/{} ({}%)".format(deformation_name, correctness_cnt_for_deformation, num_comparisons_for_deformation, 
                                            correctness_cnt_for_deformation/num_comparisons_for_deformation * 100))
...
class TrialClassificationResult:
    ...
    def __init__(self, test_trial, n_best_list_tuple):
        ...
        self.is_correct = test_trial.get_ground_truth_deformation_name() == self.closest_trial.deformation_name
    ...
    def get_correct_match_index_nbestlist(self):
        ...
        for fold_trial, score in self.n_best_list_sorted:
            if self.test_trial.deformation_name == fold_trial.deformation_name:
                return index
            index += 1    
        return -1
    ...
    def __str__(self):
        ...
        return("{} : Best match for '{}' Trial {} is '{}' Trial {} w/score: {:0.1f} ({:0.3f}s)".format(
                     correctness_str, self.test_trial.get_ground_truth_deformation_name(), self.test_trial.trial_num, 
                     self.closest_trial.deformation_name, self.closest_trial.trial_num, 
